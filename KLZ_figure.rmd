...
metadata=read_csv("metadata1.csv") %>% arrange(Individual_ID,sam2adm)
...
genus_A=t(genus[,metadata %>% filter(Death %in% c(1,0)) %>% .[,"Sample"]] %>% as.matrix() %>% prop.table(.,2) %>% .[rowSums(.>0.01)>0,]%>% .[,map_A$Sample] )
genus_D=t(genus[,metadata %>% filter(Death %in% c(1,0)) %>% .[,"Sample"]] %>% as.matrix() %>% prop.table(.,2) %>% .[rowSums(.>0.01)>0,] %>% .[,map_D$Sample])
...
env1 <- dplyr::select(map_A,Sample,Age,Gender,CORTICOSTEROID,Death,
                      ...
                      Individual_ID)
env1=env1[env1$Sample %in% rownames(genus_A),]
...
gamlss_t1 <- taxa.compare(taxtab =df1,propmed.rel="gamlss",comvar="Death",personid = "Individual_ID",
...
env1 <- dplyr::select(map_D,Sample,Age,Gender,CORTICOSTEROID,Death,
                      COMORBIDITY,Treatment,Severity_A,Copy,ANTB_CLS3_STATUS,
                      Individual_ID)
env1=env1[env1$Sample %in% rownames(genus_D),]
env1$CORTICOSTEROID=as.factor(env1$CORTICOSTEROID)
env1$Severity_A=as.factor(env1$Severity_A)
env1$Death=as.factor(env1$Death)
env1$ANTB_CLS3_STATUS=as.factor(env1$ANTB_CLS3_STATUS)
...
p1=ggplot(res_death1_recovered,aes(x=sample_size,y=log10p))+geom_boxplot()+
